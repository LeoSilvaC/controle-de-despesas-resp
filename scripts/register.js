...
function salvarDados() {
  ...
  const dados = Array.from(containers).map(container => {
    ...
    container.querySelectorAll('.controle').forEach(controle => {
      ...
      const quantidade = parseInt(controle.querySelector('.controle-contador').value) || 0;
...
function createContainer(name, produtos = {}) {
  ...
  container.innerHTML = `<div class="pessoa">
    ...
    <p class="valor-total" data-estatistica>${returnCurrency(calcularTotalItem(produtos))}</p>
  </div>`;
  ...
  btnConfirmarProduto.addEventListener('click', () => {
    const nomeProduto = novoProdutoDiv.querySelector('.nome-produto').value.trim().toLowerCase();
    const precoProduto = parseFloat(novoProdutoDiv.querySelector('.preco-produto').value);
    ...
    const botoesNovoProduto = novoProdutoElement.querySelectorAll('.controle-botao');
    botoesNovoProduto.forEach(botao => {
      botao.addEventListener('click', handleButtonBehavior);
    });

    const inputNovoProduto = novoProdutoElement.querySelector('.controle-contador');
    inputNovoProduto.addEventListener('change', () => {
      handleButtonBehavior({ target: inputNovoProduto });
    });

    const btnRemoverProduto = novoProdutoElement.querySelector('.remover-produto');
...
function atualizaTotalPessoa(container) {
  ...
  const total = calcularTotalItem(produtos);
  container.querySelector("[data-estatistica]").innerText = returnCurrency(total);
}
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function realizarPesquisa() {
    ...
    pessoas.forEach(pessoa => {
      const nomePessoa = pessoa.querySelector('.nome-pessoa').textContent.toLowerCase();
